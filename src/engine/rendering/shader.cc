...
util::Result<Shader, std::exception> Shader::create(
  ...
  if (result.is_err()) {
    return util::Result<Shader, std::exception>::Err(result.unwrap_err());
  }
  ...
  return util::Result<Shader, std::exception>::Ok(shader);
};
